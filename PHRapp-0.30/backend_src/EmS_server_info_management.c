...
// Implementation
static boolean change_server_addresses_configuration(SSL *ssl_client)   // A User Authority records transaction logs already so an Emergency Server does not do it anymore
{
	...
	sprintf(stat, "UPDATE %s SET audit_server_ip_addr = '%s', phr_server_ip_addr = '%s'", EMS__BASIC_AUTHORITY_INFO, audit_server_ip_addr, phr_server_ip_addr);
...
// A User Authority records transaction logs already so an Emergency Server does not do it anymore
static boolean change_mail_server_configuration(SSL *ssl_client)
{
	char  buffer[BUFFER_LENGTH + 1];
	char  token_name[TOKEN_NAME_LENGTH + 1];
	...
	sprintf(stat, "UPDATE %s SET mail_server_url = '%s', authority_email_address = '%s', authority_email_passwd = '%s'", 
		EMS__BASIC_AUTHORITY_INFO, mail_server_url, authority_email_address, authority_email_passwd);
...
void *server_info_management_main(void *arg)
{
	...
	char    *hosts[1];
    	...
    	bio_acc = BIO_new_accept(EMS_SERVER_INFO_MANAGEMENT_PORT);
    	if(!bio_acc)
