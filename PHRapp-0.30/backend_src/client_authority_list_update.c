...
#define CALCULATING_SSL_CERT_HASH_PATH "Client_cache/client_authority_list_update.calculating_ssl_cert_hash"
...
static void (*clear_authority_list_callback_handler)()                       = NULL;
static void (*add_authority_to_list_callback_handler)(char *authority_name)  = NULL;
static void (*clear_authority_table_callback_handler)()                      = NULL;
static void (*add_authority_to_table_callback_handler)(char *authority_name, 
	char *ip_address, boolean authority_join_flag)                       = NULL;
...
static void clear_authority_list_handler_callback();
...
static boolean connect_to_authority_list_loading_service(SSL **ssl_conn_ret);
...
static void clear_authority_list_handler_callback()
{
	if(clear_authority_list_callback_handler)
	{
		clear_authority_list_callback_handler();
	}
	else  // NULL
	{
		int_error("clear_authority_list_callback_handler is NULL");
	}
}
...
static boolean connect_to_authority_list_loading_service(SSL **ssl_conn_ret)
{
	...
	sprintf(user_auth_addr, "%s:%s", GLOBAL_user_auth_ip_addr, UA_AUTHORITY_LIST_LOADING_PORT);
	...
	void (*clear_authority_list_callback_handler_ptr)(), void (*add_authority_to_list_callback_handler_ptr)(char *authority_name))
{
	...
	clear_authority_list_callback_handler    = clear_authority_list_callback_handler_ptr;
	add_authority_to_list_callback_handler   = add_authority_to_list_callback_handler_ptr;

	SSL     *ssl_conn = NULL;
	char    buffer[BUFFER_LENGTH + 1];
	char    token_name[TOKEN_NAME_LENGTH + 1];

	char    is_end_of_authority_list_flag_str_tmp[FLAG_LENGTH + 1];
	boolean is_end_of_authority_list_flag;
	char    authority_name[AUTHORITY_NAME_LENGTH + 1];

	// Connect to User Authority
	if(!connect_to_authority_list_loading_service(&ssl_conn))
		goto ERROR;

	// Clear authority list
	clear_authority_list_handler_callback();

	while(1)
	{
		// Receive authority information
		if(SSL_recv_buffer(ssl_conn, buffer, NULL) == 0)
		{
			backend_alert_msg_handler_callback("Receiving authority information failed");
			goto ERROR;
		}

		// Get authority information tokens from buffer
		if(read_token_from_buffer(buffer, 1, token_name, is_end_of_authority_list_flag_str_tmp) 
			!= READ_TOKEN_SUCCESS || strcmp(token_name, "is_end_of_authority_list_flag") != 0)
		{
			int_error("Extracting the is_end_of_authority_list_flag failed");
		}

		is_end_of_authority_list_flag = (strcmp(is_end_of_authority_list_flag_str_tmp, "1") == 0) ? true : false;
		if(is_end_of_authority_list_flag)
			break;

		if(read_token_from_buffer(buffer, 2, token_name, authority_name) != READ_TOKEN_SUCCESS || strcmp(token_name, "authority_name") != 0)
		{
			int_error("Extracting the authority_name failed");
		}

		// Add an authority to list
		add_authority_to_list_handler_callback(authority_name);
	}
	...
	boolean authority_join_flag))
{
	...
	backend_alert_msg_callback_handler       = backend_alert_msg_callback_handler_ptr;
	backend_fatal_alert_msg_callback_handler = backend_fatal_alert_msg_callback_handler_ptr;
	clear_authority_table_callback_handler   = clear_authority_table_callback_handler_ptr;
	add_authority_to_table_callback_handler  = add_authority_to_table_callback_handler_ptr;
	...
	char    authority_join_flag_str_tmp[FLAG_LENGTH + 1];
	boolean authority_join_flag;
	...
	// Clear authority table
	clear_authority_table_handler_callback();
	...
	while(1)
	{
		...
		if(read_token_from_buffer(buffer, 4, token_name, authority_join_flag_str_tmp) != READ_TOKEN_SUCCESS || strcmp(token_name, "authority_join_flag") != 0)
		{
			int_error("Extracting the authority_join_flag failed");
		}

		authority_join_flag = (strcmp(authority_join_flag_str_tmp, "1") == 0) ? true : false;

		// Add an authority to table
		add_authority_to_table_handler_callback(authority_name, ip_address, authority_join_flag);
	}
