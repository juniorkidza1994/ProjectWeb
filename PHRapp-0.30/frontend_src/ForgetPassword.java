...
class ForgetPassword extends JDialog implements ConstantVars
{
	private static final long serialVersionUID = -1113582265863921754L;

	// Declaration of the Native C functions
	private native boolean load_user_authority_pub_key_main(String user_auth_ip_addr);
	private native boolean request_passwd_resetting_code_main(String user_auth_ip_addr, String username, boolean is_admin_flag);
	private native boolean reset_passwd_main(String user_auth_ip_addr, String username, boolean is_admin_flag, String resetting_code);
	...
	private JTextField     user_auth_ip_addr_textfield_code_requesting;
	...
	// Passwd resetting
	private JTextField     user_auth_ip_addr_textfield_passwd_resetting;
	...
	private final void init_ui_for_passwd_resetting_code_requesting()
	{
		...
		user_auth_ip_addr_textfield_code_requesting = new JTextField(TEXTFIELD_LENGTH);
		...
		upper_inner_panel.add(user_auth_ip_addr_textfield_code_requesting);
		...
		// Set focus
		user_auth_ip_addr_textfield_code_requesting.requestFocus(true);
			...
			public void actionPerformed(ActionEvent event)
			{
				SwingUtilities.invokeLater(new Runnable()
				{
		    			public void run()
					{
						...
						{
							String user_auth_ip_addr = user_auth_ip_addr_textfield_code_requesting.getText();
							...
							if(user_type.equals(user_type_code_requesting))
							{
								...
								if(request_passwd_resetting_code_main(user_auth_ip_addr, username, false))
								{
									...
									init_ui_for_passwd_resetting();
									setup_actions_for_passwd_resetting();
									pre_init_passwd_resetting_input(user_auth_ip_addr, username, false);
							...
							else if(user_type.equals(admin_type_code_requesting))
							{
								...
								if(request_passwd_resetting_code_main(user_auth_ip_addr, username, true))
								{
									...
									init_ui_for_passwd_resetting();
									setup_actions_for_passwd_resetting();
									pre_init_passwd_resetting_input(user_auth_ip_addr, username, true);
	...
	private boolean validate_passwd_resetting_code_requesting_input()
	{
		...
		m = p.matcher(user_auth_ip_addr_textfield_code_requesting.getText());
		if(m.matches() == false)
		{
	...
	{
		user_auth_ip_addr_textfield_passwd_resetting.setText(assigned_user_auth_ip_addr);
		user_auth_ip_addr_textfield_passwd_resetting.setEnabled(false);
	...
	private final void init_ui_for_passwd_resetting()
	{
		...
		user_auth_ip_addr_textfield_passwd_resetting = new JTextField(TEXTFIELD_LENGTH);
		...
		JPanel upper_inner_panel = new JPanel(new SpringLayout());
		upper_inner_panel.add(user_auth_ip_addr_label);
		upper_inner_panel.add(user_auth_ip_addr_textfield_passwd_resetting);
		upper_inner_panel.add(username_label);
		...
		// Set focus
		user_auth_ip_addr_textfield_passwd_resetting.requestFocus(true);
			...
			public void actionPerformed(ActionEvent event)
			{
				SwingUtilities.invokeLater(new Runnable()
				{
		    			public void run()
					{
						...
						{
							String user_auth_ip_addr = user_auth_ip_addr_textfield_passwd_resetting.getText();
	...
	private boolean validate_passwd_resetting_input()
	{
		...
		m = p.matcher(user_auth_ip_addr_textfield_passwd_resetting.getText());
		if(m.matches() == false)
		{
			JOptionPane.showMessageDialog(this, "Please input correct format for the IP address");
			return false;
