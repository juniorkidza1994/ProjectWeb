...
#define CALCULATING_SSL_CERT_HASH_PATH  "EmU_client_cache/EmU_client_restricted_level_phr_access_requesting.calculating_ssl_cert_hash"
...
static boolean connect_to_restricted_level_phr_access_requesting_management_service(char *emergency_server_ip_addr, char *target_authority_name, SSL **ssl_conn_ret);
...
static boolean connect_to_restricted_level_phr_access_requesting_management_service(char *emergency_server_ip_addr, char *target_authority_name, SSL **ssl_conn_ret)
{
	...
	sprintf(emergency_server_addr, "%s:%s", emergency_server_ip_addr, EMS_RESTRICTED_LEVEL_PHR_ACCESS_REQUESTING_PORT);
	...
	void (*backend_fatal_alert_msg_callback_handler_ptr)(char *alert_msg))
{
	...
	char    restricted_level_phr_access_requesting_result_flag_str_tmp[FLAG_LENGTH + 1];
	boolean restricted_level_phr_access_requesting_result_flag;

	// Connect to Emergency Server of the target authority
	if(!connect_to_restricted_level_phr_access_requesting_management_service(emergency_server_ip_addr, phr_owner_authority_name, &ssl_conn))
		goto ERROR;

	// Send request information
	write_token_into_buffer("request", RESTRICTED_LEVEL_PHR_ACCESS_REQUESTING, true, buffer);
	...
	// Send the restricted-level PHR access requesting information
	write_token_into_buffer("phr_ownername", phr_ownername, true, buffer);
	write_token_into_buffer("remote_site_phr_id", phr_id_str, false, buffer);
	write_token_into_buffer("phr_description", phr_description, false, buffer);
	write_token_into_buffer("emergency_staff_email_address", emergency_staff_email_address, false, buffer);

	if(!SSL_send_buffer(ssl_conn, buffer, strlen(buffer)))
	{
		backend_alert_msg_handler_callback("Sending the restricted-level PHR access requesting information failed");
		goto ERROR;
	}

	// Receive the restricted-level PHR access requesting result
	if(SSL_recv_buffer(ssl_conn, buffer, NULL) == 0)
	{
		backend_alert_msg_handler_callback("Receiving the restricted-level PHR access requesting result failed");
		goto ERROR;
	}

	// Get a restricted-level PHR access requesting result flag token from buffer
	if(read_token_from_buffer(buffer, 1, token_name, restricted_level_phr_access_requesting_result_flag_str_tmp) 
		!= READ_TOKEN_SUCCESS || strcmp(token_name, "restricted_level_phr_access_requesting_result_flag") != 0)
	{
		int_error("Extracting the restricted_level_phr_access_requesting_result_flag failed");
	}

	restricted_level_phr_access_requesting_result_flag = (strcmp(restricted_level_phr_access_requesting_result_flag_str_tmp, "1") == 0) ? true : false;
	if(!restricted_level_phr_access_requesting_result_flag)
	{
	...
	char *phr_description, void (*backend_alert_msg_callback_handler_ptr)(char *alert_msg), void (*backend_fatal_alert_msg_callback_handler_ptr)(char *alert_msg))
{
	...
	char    restricted_level_phr_access_request_cancellation_result_flag_str_tmp[FLAG_LENGTH + 1];
	boolean restricted_level_phr_access_request_cancellation_result_flag;
	...
	// Send request information
	write_token_into_buffer("request", RESTRICTED_LEVEL_PHR_ACCESS_REQUEST_CANCELLATION, true, buffer);
	...
	// Send the restricted-level PHR access request cancellation information
	write_token_into_buffer("phr_ownername", phr_ownername, true, buffer);
	write_token_into_buffer("remote_site_phr_id", phr_id_str, false, buffer);
	write_token_into_buffer("phr_description", phr_description, false, buffer);
	...
	if(!SSL_send_buffer(ssl_conn, buffer, strlen(buffer)))
	{
		backend_alert_msg_handler_callback("Sending the restricted-level PHR access request cancellation information failed");
		goto ERROR;
	}

	// Receive the restricted-level PHR access request cancellation result
	if(SSL_recv_buffer(ssl_conn, buffer, NULL) == 0)
	{
		backend_alert_msg_handler_callback("Receiving the restricted-level PHR access request cancellation result failed");
		goto ERROR;
	}

	// Get a restricted-level PHR access request cancellation result flag token from buffer
	if(read_token_from_buffer(buffer, 1, token_name, restricted_level_phr_access_request_cancellation_result_flag_str_tmp) 
		!= READ_TOKEN_SUCCESS || strcmp(token_name, "restricted_level_phr_access_request_cancellation_result_flag") != 0)
	{
		int_error("Extracting the restricted_level_phr_access_request_cancellation_result_flag failed");
	}

	restricted_level_phr_access_request_cancellation_result_flag = (strcmp(restricted_level_phr_access_request_cancellation_result_flag_str_tmp, "1") == 0) ? true : false;
	if(!restricted_level_phr_access_request_cancellation_result_flag)
	{
		char error_msg[ERR_MSG_LENGTH + 1];
