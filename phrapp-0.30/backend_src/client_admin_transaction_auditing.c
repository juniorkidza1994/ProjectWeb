...
#define CALCULATING_SSL_CERT_HASH_PATH "Client_cache/client_admin_transaction_auditing.calculating_ssl_cert_hash"
...
static void (*add_transaction_admin_login_log_to_table_callback_handler)(
	char *date_time, char *ip_address, boolean is_logout_flag)             = NULL;

static void (*add_transaction_system_login_log_to_table_callback_handler)(
	char *date_time, char *username, char *user_authority_name, 
	boolean is_admin_flag, char *ip_address, boolean is_logout_flag)       = NULL;
...
static void add_transaction_admin_login_log_to_table_handler_callback(char *date_time, char *ip_address, boolean is_logout_flag);
static void add_transaction_system_login_log_to_table_handler_callback(char *date_time, char *username, 
...
static void audit_transaction_login_log(char *request, boolean audit_admin_log_flag);
static void audit_transaction_event_log(char *request);
...
static void add_transaction_admin_login_log_to_table_handler_callback(char *date_time, char *ip_address, boolean is_logout_flag)
{
	if(add_transaction_admin_login_log_to_table_callback_handler)
	{
		add_transaction_admin_login_log_to_table_callback_handler(date_time, ip_address, is_logout_flag);
	}
	else  // NULL
	{
		int_error("add_transaction_admin_login_log_to_table_callback_handler is NULL");
	}
}
...
static void add_transaction_system_login_log_to_table_handler_callback(char *date_time, char *username, 
	char *user_authority_name, boolean is_admin_flag, char *ip_address, boolean is_logout_flag)
{
	if(add_transaction_system_login_log_to_table_callback_handler)
	{
		add_transaction_system_login_log_to_table_callback_handler(date_time, username, user_authority_name, is_admin_flag, ip_address, is_logout_flag);
	}
	else  // NULL
	{
		int_error("add_transaction_system_login_log_to_table_callback_handler is NULL");
	}
}
	...
	boolean is_object_owner_admin_flag, char *actor_ip_address)
{
	...
	{
		add_transaction_event_log_to_table_callback_handler(date_time, actor_name, actor_authority_name, is_actor_admin_flag, 
			object_description, event_description, object_owner_name, object_owner_authority_name, is_object_owner_admin_flag, actor_ip_address);
	}
	else  // NULL
	{
...
static void audit_transaction_login_log(char *request, boolean audit_admin_log_flag)
{
	...
	while(1)
	{
		...
		if(audit_admin_log_flag)
		{
			...
			add_transaction_admin_login_log_to_table_handler_callback(date_time, ip_address, is_logout_flag);
		}
		else
		{
			...
			if(read_token_from_buffer(buffer, 5, token_name, username) != READ_TOKEN_SUCCESS || strcmp(token_name, "username") != 0)
			{
				int_error("Extracting the username failed");
			}

			if(read_token_from_buffer(buffer, 6, token_name, is_admin_flag_str_tmp) != READ_TOKEN_SUCCESS || strcmp(token_name, "is_admin_flag") != 0)
			{
			...
			// Add a transaction login log to a table
			add_transaction_system_login_log_to_table_handler_callback(date_time, username, GLOBAL_authority_name, is_admin_flag, ip_address, is_logout_flag);
		}
	...
	if(!found_log_flag)
	{
		backend_alert_msg_handler_callback("Transaction log not found");
	}
	...
	char *alert_msg), void (*add_transaction_admin_login_log_to_table_callback_handler_ptr)(char *date_time, char *ip_address, boolean is_logout_flag))
{
	...
	add_transaction_admin_login_log_to_table_callback_handler = add_transaction_admin_login_log_to_table_callback_handler_ptr;

	char request[BUFFER_LENGTH + 1];

	// Transaction log auditing request information
	write_token_into_buffer("request_type", ADMIN_LOGIN_LOG_AUDITING, true, request);
	write_token_into_buffer("audit_all_transactions_flag", "1", false, request);

	audit_transaction_login_log(request, true);
}
	...
	boolean is_admin_flag, char *ip_address, boolean is_logout_flag))
{
	...
	backend_alert_msg_callback_handler                         = backend_alert_msg_callback_handler_ptr;
	backend_fatal_alert_msg_callback_handler                   = backend_fatal_alert_msg_callback_handler_ptr;
	add_transaction_system_login_log_to_table_callback_handler = add_transaction_system_login_log_to_table_callback_handler_ptr;

	char request[BUFFER_LENGTH + 1];

	// Transaction log auditing request information
	write_token_into_buffer("request_type", SYSTEM_LOGIN_LOG_AUDITING, true, request);
	write_token_into_buffer("audit_all_transactions_flag", "1", false, request);

	audit_transaction_login_log(request, false);
}
	...
	boolean is_object_owner_admin_flag, char *actor_ip_address))
{
	...
	// Transaction log auditing request information
	write_token_into_buffer("request_type", ADMIN_EVENT_LOG_AUDITING, true, request);
	write_token_into_buffer("audit_all_transactions_flag", "1", false, request);

	audit_transaction_event_log(request);
}
	...
	boolean is_object_owner_admin_flag, char *actor_ip_address))
{
	...
	// Transaction log auditing request information
	write_token_into_buffer("request_type", SYSTEM_EVENT_LOG_AUDITING, true, request);
	write_token_into_buffer("audit_all_transactions_flag", "1", false, request);
