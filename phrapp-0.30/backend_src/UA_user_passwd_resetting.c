...
#define PASSWD_RESETTING_REQUESTING_INFO_CIPHERTEXT_PATH   	"UA_cache/UA_user_passwd_resetting.passwd_resetting_requesting_info_ciphertext"
#define PASSWD_RESETTING_REQUESTING_INFO_PLAINTEXT_PATH    	"UA_cache/UA_user_passwd_resetting.passwd_resetting_requesting_info_plaintext"

#define PASSWD_RESETTING_CODE_REQUESTING_RESULT_CIPHERTEXT_PATH "UA_cache/UA_user_passwd_resetting.passwd_resetting_code_requesting_result_ciphertext"
#define PASSWD_RESETTING_CODE_REQUESTING_RESULT_PLAINTEXT_PATH  "UA_cache/UA_user_passwd_resetting.passwd_resetting_code_requesting_result_plaintext"

#define PASSWD_RESETTING_RESULT_CIPHERTEXT_PATH 		"UA_cache/UA_user_passwd_resetting.passwd_resetting_result_ciphertext"
#define PASSWD_RESETTING_RESULT_PLAINTEXT_PATH  		"UA_cache/UA_user_passwd_resetting.passwd_resetting_result_plaintext"

#define SALTED_PASSWD_HASH_PATH      				"UA_cache/UA_user_passwd_resetting.salted_passwd_hash"

#define SSL_CERT_PRIV_KEY_PATH       				"UA_cache/UA_user_passwd_resetting.ssl_cert_priv_key"
#define SSL_CERT_REQ_PATH            				"UA_cache/UA_user_passwd_resetting.ssl_cert_req"
#define ENC_SSL_CERT_PATH            				"UA_cache/UA_user_passwd_resetting.enc_ssl_cert"
#define FULL_ENC_SSL_CERT_PATH       				"UA_cache/UA_user_passwd_resetting.full_enc_ssl_cert"
#define FULL_ENC_SSL_CERT_HASH_PATH  				"UA_cache/UA_user_passwd_resetting.full_enc_ssl_cert_hash"

#define CPABE_PRIV_KEY_PATH          				"UA_cache/UA_user_passwd_resetting.cpabe_priv_key"
#define ENC_CPABE_PRIV_KEY_PATH      				"UA_cache/UA_user_passwd_resetting.cpabe_priv_key_ciphertext"
#define ENC_CPABE_PRIV_KEY_HASH_PATH 				"UA_cache/UA_user_passwd_resetting.cpabe_priv_key_ciphertext_hash"

// Local Function Prototypes
static boolean record_transaction_log(BIO *bio_client, char *username, boolean is_admin_flag, char *event_description);
...
static boolean reset_user_passwd(BIO *bio_client, MYSQL *db_conn, unsigned int user_id, char *info_exchange_passwd, 
	const char *passwd_resetting_result_plaintext_path, const char *passwd_resetting_result_ciphertext_path, 
	const char *salted_passwd_hash_path, const char *ssl_cert_priv_key_path, const char *ssl_cert_req_path, 
	const char *enc_ssl_cert_path, const char *full_enc_ssl_cert_path, const char *full_enc_ssl_cert_hash_path, 
	const char *cpabe_priv_key_path, const char *enc_cpabe_priv_key_path, const char *enc_cpabe_priv_key_hash_path);

static boolean reset_admin_passwd(BIO *bio_client, MYSQL *db_conn, unsigned int admin_id, char *info_exchange_passwd, 
	const char *passwd_resetting_result_plaintext_path, const char *passwd_resetting_result_ciphertext_path, 
	const char *salted_passwd_hash_path, const char *ssl_cert_priv_key_path, const char *ssl_cert_req_path, 
	const char *enc_ssl_cert_path, const char *full_enc_ssl_cert_path, const char *full_enc_ssl_cert_hash_path);
...
static boolean record_transaction_log(BIO *bio_client, char *username, boolean is_admin_flag, char *event_description)
{
	...
	write_token_into_buffer("actor_name", PASSWD_FORGETTOR_NAME, true, buffer);
...
static boolean respond_passwd_resetting_code(BIO *bio_client, char *username, boolean is_admin_flag, char *info_exchange_passwd)
{
	...
	if(is_admin_flag)
	{
		sprintf(stat, "UPDATE %s SET passwd_resetting_code = '%s' WHERE admin_id = %u", UA__ADMINS, random_passwd_resetting_code, user_or_admin_id);
	}
	else
	{
		sprintf(stat, "UPDATE %s SET passwd_resetting_code = '%s' WHERE user_id = %u", UA__USERS, random_passwd_resetting_code, user_or_admin_id);
	}
	...
	record_transaction_log(bio_client, username, is_admin_flag, PASSWD_RESETTING_CODE_REQUESTING_MSG);
	return true;
	...
	const char *enc_ssl_cert_path, const char *full_enc_ssl_cert_path, const char *full_enc_ssl_cert_hash_path)
{
	...
	if(!row)
	{
		...
		if(!write_token_into_file("error_msg", "Admin does not exist", false, passwd_resetting_result_plaintext_path))
			int_error("Writing the error_msg failed");
...
static boolean respond_passwd_resetting(BIO *bio_client, char *username, boolean is_admin_flag, char *resetting_code, char *info_exchange_passwd)
{
	...
	if(is_admin_flag)
	{
		sprintf(stat, "SELECT admin_id, passwd_resetting_code FROM %s WHERE username LIKE '%s' COLLATE latin1_general_cs", UA__ADMINS, username);
	}
	else  // Normal user
	{
		sprintf(stat, "SELECT user_id, passwd_resetting_code FROM %s WHERE username LIKE '%s' COLLATE latin1_general_cs", UA__USERS, username);
	}
	...
	if(strcmp(resetting_code_cmp, resetting_code) != 0)
	{
		...
		// Record a transaction log
		record_transaction_log(bio_client, username, is_admin_flag, INVALID_PASSWD_RESETTING_CODE_MSG);
		goto ERROR;
	}
	...
	if(is_admin_flag)
	{
		...
		if(!reset_admin_passwd(bio_client, db_conn, user_or_admin_id, info_exchange_passwd, PASSWD_RESETTING_RESULT_PLAINTEXT_PATH, 
	...
	else
	{
		...
		if(!reset_user_passwd(bio_client, db_conn, user_or_admin_id, info_exchange_passwd, PASSWD_RESETTING_RESULT_PLAINTEXT_PATH, 
			PASSWD_RESETTING_RESULT_CIPHERTEXT_PATH, SALTED_PASSWD_HASH_PATH, SSL_CERT_PRIV_KEY_PATH, SSL_CERT_REQ_PATH, ENC_SSL_CERT_PATH, 
			FULL_ENC_SSL_CERT_PATH, FULL_ENC_SSL_CERT_HASH_PATH, CPABE_PRIV_KEY_PATH, ENC_CPABE_PRIV_KEY_PATH, ENC_CPABE_PRIV_KEY_HASH_PATH))
		{
			goto ERROR;
		}
	}
	...
	if(is_admin_flag)
	{
		sprintf(stat, "UPDATE %s SET passwd_resetting_code = '' WHERE admin_id = %u", UA__ADMINS, user_or_admin_id);
	}
	else
	{
		sprintf(stat, "UPDATE %s SET passwd_resetting_code = '' WHERE user_id = %u", UA__USERS, user_or_admin_id);
	}
	...
	// Record a transaction log
	record_transaction_log(bio_client, username, is_admin_flag, PASSWD_RESETTING_SUCCEEDED_MSG);
	return true;
...
static boolean process_request(BIO *bio_client)
{
	...
	// Decrypt the password resetting requesting information with the User Authority's private key
	if(!smime_decrypt_with_cert(PASSWD_RESETTING_REQUESTING_INFO_CIPHERTEXT_PATH, 
		PASSWD_RESETTING_REQUESTING_INFO_PLAINTEXT_PATH, UA_CERTFILE_PATH, UA_CERTFILE_PASSWD, err_msg))
	{
...
void *user_passwd_resetting_main(void *arg)
{
	...
	BIO *bio_client = NULL;
    	...
    	bio_acc = BIO_new_accept(UA_USER_PASSWD_RESETTING_PORT);
    	if(!bio_acc)
