...
#define VERIFICATION_INFO_CIPHERTEXT_PATH   "EmU_client_cache/EmU_client_login_authentication.verification_info_ciphertext"
#define VERIFICATION_INFO_PLAINTEXT_PATH    "EmU_client_cache/EmU_client_login_authentication.verification_info_plaintext"
#define VERIFICATION_RESULT_CIPHERTEXT_PATH "EmU_client_cache/EmU_client_login_authentication.verification_result_ciphertext"
#define VERIFICATION_RESULT_PLAINTEXT_PATH  "EmU_client_cache/EmU_client_login_authentication.verification_result_plaintext"

#define SSL_CERT_CIPHERTEXT_PATH            "EmU_client_cache/EmU_client_login_authentication.ssl_cert_ciphertext"
#define SSL_CERT_HASH_CIPHERTEXT_PATH       "EmU_client_cache/EmU_client_login_authentication.ssl_cert_hash_ciphertext"
#define SSL_CERT_HASH_PLAINTEXT_PATH        "EmU_client_cache/EmU_client_login_authentication.ssl_cert_hash_plaintext"
#define CALCULATING_SSL_CERT_HASH_PATH      "EmU_client_cache/EmU_client_login_authentication.calculating_ssl_cert_hash"
...
static void (*basic_info_ret_callback_handler)(char *email_address, char *authority_name)        = NULL;
...
static void basic_info_ret_handler_callback(char *email_address, char *authority_name);
...
static void basic_info_ret_handler_callback(char *email_address, char *authority_name)
{
	...
	{
		basic_info_ret_callback_handler(email_address, authority_name);
	}
	else  // NULL
	{
...
static boolean send_authentication_request(BIO *bio_conn, boolean is_admin_flag, char *username, char *passwd, char *key_exchange_passwd)
{
	...
	// Encrypt the verification information with the Emergency Staff Authority's public key
	if(!smime_encrypt_with_cert(VERIFICATION_INFO_PLAINTEXT_PATH, VERIFICATION_INFO_CIPHERTEXT_PATH, ESA_PUB_CERTFILE_PATH, err_msg))
	{
	...
	char *mail_server_url_ret, char *authority_email_address_ret, char *authority_email_passwd_ret)
{
	...
	if(read_token_from_buffer(buffer, 3, token_name, mail_server_url_ret) != READ_TOKEN_SUCCESS || strcmp(token_name, "mail_server_url") != 0)
		int_error("Extracting the mail_server_url failed");

	if(read_token_from_buffer(buffer, 4, token_name, authority_email_address_ret) != READ_TOKEN_SUCCESS || strcmp(token_name, "authority_email_address") != 0)
		int_error("Extracting the authority_email_address failed");

	if(read_token_from_buffer(buffer, 5, token_name, authority_email_passwd_ret) != READ_TOKEN_SUCCESS || strcmp(token_name, "authority_email_passwd") != 0)
	...
	void (*ssl_cert_hash_ret_callback_handler_ptr)(char *ssl_cert_hash))
{
	...
	sprintf(emergency_staff_auth_addr, "%s:%s", emergency_staff_auth_ip_addr, ESA_USER_AUTHENTICATION_PORT);
	...
	ctx = setup_client_ctx(SSL_CERT_PATH, passwd, EMU_ROOT_CA_ONLY_CERT_CERTFILE_PATH);
	...
	host[0] = EMERGENCY_STAFF_AUTH_CN;
	...
	if(!obtain_basic_info_for_user(ssl_conn, email_address, authority_name))
        	goto ERROR_AT_SSL_LAYER;

	basic_info_ret_handler_callback(email_address, authority_name);
	...
	void (*ssl_cert_hash_ret_callback_handler_ptr)(char *ssl_cert_hash))
{
	...
	// Obtain basic information
	if(!obtain_basic_info_for_admin(ssl_conn, email_address, authority_name, mail_server_url, authority_email_address, authority_email_passwd))
	{
        	goto ERROR_AT_SSL_LAYER;
	}
