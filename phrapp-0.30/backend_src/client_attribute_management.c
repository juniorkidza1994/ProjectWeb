...
#define CALCULATING_SSL_CERT_HASH_PATH "Client_cache/client_attribute_management.calculating_ssl_cert_hash"
...
static boolean connect_to_attribute_management_service(SSL **ssl_conn_ret);
...
static boolean connect_to_attribute_management_service(SSL **ssl_conn_ret)
{
	...
	sprintf(user_auth_addr, "%s:%s", GLOBAL_user_auth_ip_addr, UA_ATTRIBUTE_MANAGEMENT_PORT);
	...
	void (*backend_fatal_alert_msg_callback_handler_ptr)(char *alert_msg))
{
	...
	char    attribute_registration_result_flag_str_tmp[FLAG_LENGTH + 1];    // "0" or "1"
	boolean attribute_registration_result_flag;

	// Connect to User Authority
	if(!connect_to_attribute_management_service(&ssl_conn))
		goto ERROR;

	// Send request information
	write_token_into_buffer("request", ATTRIBUTE_REGISTRATION, true, buffer);
	...
	// Send attribute registration information
	write_token_into_buffer("attribute_name", attribute_name, true, buffer);
	write_token_into_buffer("is_numerical_attribute_flag", (is_numerical_attribute_flag) ? "1" : "0", false, buffer);

	if(!SSL_send_buffer(ssl_conn, buffer, strlen(buffer)))
	{
		backend_alert_msg_handler_callback("Sending attribute registration information failed");
		goto ERROR;
	}

	// Receive attribute registration result flag
	if(SSL_recv_buffer(ssl_conn, buffer, NULL) == 0)
	{
		backend_alert_msg_handler_callback("Receiving attribute registration result failed");
		goto ERROR;
	}

	// Get an attribute registration result flag token from buffer
	if(read_token_from_buffer(buffer, 1, token_name, attribute_registration_result_flag_str_tmp) 
		!= READ_TOKEN_SUCCESS || strcmp(token_name, "attribute_registration_result_flag") != 0)
	{
		int_error("Extracting the attribute_registration_result_flag failed");
	}

	attribute_registration_result_flag = (strcmp(attribute_registration_result_flag_str_tmp, "1") == 0) ? true : false;
	if(!attribute_registration_result_flag)
	{
	...
	void(*backend_fatal_alert_msg_callback_handler_ptr)(char *alert_msg))
{
	...
	char    buffer[BUFFER_LENGTH + 1];
	char    token_name[TOKEN_NAME_LENGTH + 1];
	char    attribute_removal_result_flag_str_tmp[FLAG_LENGTH + 1];    // "0" or "1"
	boolean attribute_removal_result_flag;
	...
	// Send request information
	write_token_into_buffer("request", ATTRIBUTE_REMOVAL, true, buffer);
	...
	// Send attribute removal information
	write_token_into_buffer("attribute_name", attribute_name, true, buffer);

	if(!SSL_send_buffer(ssl_conn, buffer, strlen(buffer)))
	{
		backend_alert_msg_handler_callback("Sending attribute removal information failed");
		goto ERROR;
	}

	// Receive attribute removal result flag
	if(SSL_recv_buffer(ssl_conn, buffer, NULL) == 0)
	{
		backend_alert_msg_handler_callback("Receiving attribute removal result failed");
		goto ERROR;
	}

	// Get an attribute removal result flag token from buffer
	if(read_token_from_buffer(buffer, 1, token_name, attribute_removal_result_flag_str_tmp) 
		!= READ_TOKEN_SUCCESS || strcmp(token_name, "attribute_removal_result_flag") != 0)
	{
		int_error("Extracting the attribute_removal_result_flag failed");
	}

	attribute_removal_result_flag = (strcmp(attribute_removal_result_flag_str_tmp, "1") == 0) ? true : false;
	if(!attribute_removal_result_flag)
	{
		char error_msg[ERR_MSG_LENGTH + 1];
