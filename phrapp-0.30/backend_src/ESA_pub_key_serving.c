...
static boolean serve_pub_key(BIO *bio_client)
{
	...
	{
		fprintf(stderr, "Sending an emergency staff authority's public key failed\n");
		return false;
	}
...
void *pub_key_serving_main(void *arg)
{
	...
	BIO *bio_client = NULL;
    	...
    	bio_acc = BIO_new_accept(ESA_PUB_KEY_SERVING_PORT);
    	if(!bio_acc)
		...
		// Serve an emergency staff authority's public key
