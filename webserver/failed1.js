...
if(cluster.isMaster){
    ...
    // var proxy1 = proxyMiddleware('/', {
    //             // target: "http://" + addresses+':3001',
    //             target: "http://" + addresses+':3001',
    //             ws : true,
...
//    console.log(list_server);
  ...
  //    console.log(array_key[i]);
    ...
    if(!isBusy)
      res.redirect("http://192.168.174.138:"+list_server.get(array_key[i]));
    else
   ...
   // app.use(proxy);        
    ...
    for (var i = 1; i <= 2; i += 1) {
        ...
        status_server.set(workers[i].process.pid + "", 0);
        
        workers[i].send(port_worker+i);
        list_server.set(workers[i].process.pid+ "", port_worker+i);
...
//        console.log(status_server);
    ...
    cluster.on('exit', function (worker) {
        status_server.remove(worker.pid);
        list_server.remove(worker.pid);
        // Replace the dead worker, we're not sentimental
        console.log('Worker %d died :(', worker.pid);
        var temp_worker = cluster.fork();

        list_server.set(temp_worker.pid+"", port_worker+i);
        status_server.set(temp_worker.pid + "", 0);
    });
    ...
    for(var i = 1; i <= 2; i++){
      ...
      // we only want to intercept messages that have a chat property
          if (msg.cmd == "login") {
            console.log("Child :" + msg.id + " has login");
            status_server.set(msg.id + "", 1);
            // console.log(msg.id);
            // console.log(status_server.get(msg.id));
          }
          else if(msg.cmd == "exit"){
            status_server[msg.id] = 0;
          }
        });
    }
}
  ...
  io.on('connection', function(socket){
    ...
    passport.use(new LocalStrategy(
      ...
      function(req, username, password, done) {
        ...
        console.log(req.user);
        console.log("Login With CHILD PROCESS " + process.pid);
    ...
    */
    ...
    // });
      ...
      console.log("User Connect in childe ID : " + process.pid);
      process.send({cmd:"login",id:process.pid});
  //    isBusy = true;
    //  console.log(isBusy);
      socket.on('disconnect', function(){
        console.log("User Disconnect in childe ID : " + process.pid);
      //  process.send({cmd:"login",id:process.pid});
     //   isBusy = false;
     //   console.log(isBusy);
      });
